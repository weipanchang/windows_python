...
def read_in_line():
    try:
        ...
        #     logging.error("User press ctrl-c")
        #     print("\nNo Stock Ticket Input. Abort!\n")

    except EOFError:
        logging.info("\nUser press ctrl-c")
        print("\nUser press ctrl-c, Abort!\n")
        sys.exit()

    if not stocks.isprintable():
        logging.info("\nNon Printable Input, Abort!")
        sys.exit()

    if len(stocks) > 0 and stocks[0] == " ":
#        if stocks[0] == " ":
        logging.info("\nBlank Input, Abort!")
        sys.exit()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help='Manual Input Stock Symbol List'
    )

    args = parser.parse_args()

    if len(args.stock_lst) > 0:
        sys.stdout = Logger()
        logging.log(logging.INFO, "In Line Variable")
        for stock in args.stock_lst:
#            logging.log(logging.INFO, "stock = %s" %args.stock_lst)
            main()
    else:
        stocks = read_in_line()
        if len(stocks) > 0:
