...
def extract_lines_from_pdf(path: str) -> List[str]:
    ...
    """
    if not os.path.isfile(downloadPath+cHase_pdf_file ):
        logger.error("cHase_file_namenot found at: %s", downloadPath+cHase_pdf_file )
        sys.exit(1)
    ...
    if _HAS_PDFPLUMBER:
        try:
            ...
            with pdfplumber.open(downloadPath+cHase_pdf_file) as pdf:
    ...
    logger.error("No cHase_file_nameextraction library available. Install pdfplumber or PyPDF2.")
    sys.exit(1)
...
def main():
    ...
    all_lines = extract_lines_from_pdf(cHase_pdf_file)
    if not all_lines:
