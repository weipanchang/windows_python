...
random_server_list=  random.sample(vpn_server_names, 16)

print (random_server_list)
out = open(os.path.expanduser( '~' ) + "\\Documents\\Python Scripts\\"+"random_server_list.txt", "wb")
out.write(str(random_server_list).encode()) # write text of page
out.close()

user_pass_pairs =  [line for line in open("UserName_Password.txt", "r")][7:]
#n =  len(vpn_server_names)
user_pass_pairs=  random.sample(user_pass_pairs, 16)
print (user_pass_pairs)
out = open(os.path.expanduser( '~' ) + "\\Documents\\Python Scripts\\"+"user_pass_pairs.txt", "wb")
out.write(str(user_pass_pairs).encode()) # write text of page
out.close()
...
#    user,password = user_pass_pair
    ...
    if i == 0:
        ...
        subprocess.run(["python", os.path.expanduser( '~' ) + "\\Documents\\Python Scripts\\" + "S-stock_batch_data_tipranks_2.py", json.dumps(userpass)])
